#!/usr/bin/env python3
"""Interactive uploader for recorded leorover sessions on Hugging Face."""
...
def sanitize_repo_name(value: str) -> str:
    ...
    return cleaned or "leorover-session"
...
def repo_card_text(summary: SessionSummary, repo_id: str, include_raw: bool) -> str:
    ...
    return textwrap.dedent(
        ...
        This dataset repo was uploaded from the leorover recording stack.
        ...
        This repo contains one selected leorover session only. The repo name intentionally matches the session folder name.
        """
    ).strip() + "\n"
...
def stage_upload_folder(summary: SessionSummary, *, repo_id: str, include_raw: bool) -> Path:
    ...
    tmp_dir = Path(tempfile.mkdtemp(prefix="leorover_hf_upload_"))
    ...
    *,
    ...
    try:
        ...
        api.upload_folder(
            ...
            repo_type="dataset",
            token=token,
            commit_message=f"Upload leorover session {summary.session_name}",
            commit_description=(
...
class SessionUploaderApp:
    ...
    def _build(self) -> None:
        self.root.title("leorover Hugging Face Session Uploader")
